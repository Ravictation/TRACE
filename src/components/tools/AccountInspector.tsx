...
export default function AccountInspector() {
  ...
  return (
    <div className="animate-slide-in">
      <div className="mb-3">
        <h3 className="font-mono text-sm font-bold tracking-wider text-text">👤 ACCOUNT INSPECTOR</h3>
        <p className="font-mono text-xs text-muted">Profile lookup for {acc.handle}</p>
      </div>
      ...
      <div className="rounded-md border border-border bg-panel p-4">
        <div className="mb-3 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 font-semibold text-text">
              {acc.name}
              {acc.isVerified ? (
                <svg viewBox="0 0 24 24" className="size-4 fill-accent" aria-label="Verified">
        ...
        <div className="border-t border-border pt-3">
          ...
          {acc.recentPosts.map((p) => (
            <div key={p.text} className="mb-1.5 rounded border border-border bg-panel2 p-2 text-xs text-text">
              {p.text}
      ...
      <div className="mt-3">
        <div className="mb-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-muted">
          What did you notice? (click to mark)
        </div>
